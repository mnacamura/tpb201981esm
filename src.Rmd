...
list( # TAM, IBM, and statistical model
    ...
    , "$\\eta^\\prime$" = "Rate of horizontal transmission, $h\\eta$, rescaled by $b$. $\\eta^\\prime = h\\eta/b$."
    ...
    , "$\\tilde{\\chi}$" = "Probability that each trait is transmitted from the virtual teacher to each group member in each within-group contact."
    , "$\\tilde{h}$" = "Number of within-group contacts per time step."
    , "$\\tilde{\\eta}$" = "Probability that each trait is transmitted from a donor to a recipient in another group in each between-group contact."
