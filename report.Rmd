...
to observe $k_i$ is, from the TAM spectrum, given by \begin{equation}
\label{eq:normalized-TAM}
p(k_i \mid D, \mu, b, h, \beta, \eta) = \frac{\xi_{k_i}}{\sum_{k=1}^D \xi_k}.
\end{equation}
...
If one sees the data $k_1^m$ as a popularity--frequency table
\begin{equation}
\label{eq:data-as-table}
\left\{ (k, m_k) \mid k \in \mathbb{N}, m_k = \#\{k^\prime \in k_1^m \mid k^\prime = k\} \right\},
\end{equation}
\cref{eq:log-likelihood} is represented as
\begin{align}
\label{eq:log-likelihood-for-table}
\ell(k_1^m \mid D, \lambda, \theta) =& -\sum_{k=1}^\infty m_k \log k + \sum_{k=1}^\infty m_k k
\log \lambda + m \log\Gamma(D + 1) + \sum_{k=1}^\infty m_k \log\Gamma(D + \theta - k) \nonumber \\
&- \sum_{k=1}^\infty m_k \log\Gamma(D + 1 - k) - m \log\Gamma(D + \theta)
- m \log Z(D, \lambda, \theta).
\end{align}
Note that $\sum_{k=1}^\infty m_k = m$ holds true.
Since \cref{eq:log-likelihood-for-table} is more computationally efficient than
\cref{eq:log-likelihood}, we use \cref{eq:log-likelihood-for-table} in practice.

# Bayesian MCMC

Using \cref{eq:log-likelihood-for-table}, we estimated the parameters for each of data
...
\Cref{tab:mcmc-results-1} shows the summary of obtained posterior distributions
