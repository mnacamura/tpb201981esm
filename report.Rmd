...
data_sets$IBM <- local({
    c( paste("IBM_island_t=", 140000 + 20000 * 1:3, sep = "")
     , paste("IBM_lattice_t=", 140000 + 20000 * 1:3, sep = "")
     ) %>% purrr::map(function(name) {
...
for the artificial data sets extracted from our IBM (IBM island and IBM
lattice).
From each simulation result of IBM island and lattice used in the main text,
we took three population snapshots at time steps 160,000, 180,000, and
200,000 for the check.
...
knitr::kable(summary_IBM, escape = FALSE, booktabs = TRUE
    ...
    group_rows("IBM island t=160000", 1, 5) %>%
    group_rows("IBM island t=180000", 6, 10) %>%
    group_rows("IBM island t=200000", 11, 15) %>%
    group_rows("IBM lattice t=160000", 16, 20) %>%
    group_rows("IBM lattice t=180000", 21, 25) %>%
    group_rows("IBM lattice t=200000", 26, 30)
```

```{r plot_IBM, fig.cap = "\\label{fig:plot-IBM}Posterior predictions of TAM specta: artificial data (black points), posterior means (red lines), and 95\\% credible intervals (transparent red ribbons)."}
