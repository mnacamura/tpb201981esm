...
sets (Aurignacian beads, Neolithic pottery, Neolithic ornaments, and
California basketry) by means of Bayesian MCMC (Stan, version 2.18.2).
Performing MCMC, we employed NUTS algorithm with tuning parameters
...
knitr::kable(summary_df, escape = FALSE, booktabs = TRUE
    ...
    group_rows("Neolithic pottery", 5, 8) %>%
    group_rows("Neolithic ornaments", 9, 12) %>%
    group_rows("California basketry", 13, 16)
```

\Cref{fig:plot} shows the posterior prediction of TAM spectrum for each data
set.The predictions were obtained from \cref{eq:normalized-TAM} scaled by the
number of cultural elements and the posterior distributions of $\lambda$ and
$\theta$. Many data points are out of 95\% credible intervals, implying that
the empirical data are far noisier than the theory.
